            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                ...
                <form class="form-inline my-2 my-lg-0 search-btn" method="GET" action="blog.php">
                    <input class="form-control mr-sm-2" type="search" name="search" placeholder="Search"
                        aria-label="Search">
                    <button class="btn btn-outline-primary btn-primary my-2 my-sm-0" name="submit">Search</button>
                </form>
            </div>
        </div>
    </nav>
        ...
        <div class="row py-3">
            <div class="col-sm-8 cms-header">
                ...
                if (isset($_GET['comment'])) {
                    ...
                    if ($comment == "empty") {
                        echo '<div class="display-error">You have not filled the comment form!</div>';
                    } elseif ($comment == "characterError") {
                        echo '<div class="display-error">You have unnecessary character in the comment form!</div>';
                    } elseif ($comment == "emaileroor") {
                        echo '<div class="display-error">There is an invalid email!</div>';
                    } elseif ($comment == "success") {
                        echo '<div class="display-error">You have been Submmited comment successfully!</div>';
                    } else if ($comment == "sqlerror") {
                ...
                ?>
                <!--------------------- showing the data from table for individual row  ---------------------------------->
                <?php
                ...
                <div class="card mb-2">
                    ...
                    <div class="card-body">
                        ...
                        <span class="dateandtime mb-2">Written by:<?php echo  $author; ?> <?php echo  $date; ?> </span>
                        <small class="comment"><span>Comments: <?php ApprovedComments($id); ?></span></small>
                        <p class="card-text"><?php echo  $post; ?></p>
                        <a href="fullpost.php?id=<?php echo  $blog_id; ?>" class="btn btn-primary">Read More</a>
                    </div>
                </div>
                <?php
                        }
                ...
                <?php
                    ...
                    /* Full post code after search query  */

                    //the default result for search 
                    // 
                ...
                <?php
                            ...
                            $stmt = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($stmt, $sql)) {
                                echo "There is sql error while loading comments!";
                            } else {
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    echo '
                                        <div class="card card-container display-comment mb-2">
                                        <div class="media">
                                            <img src="images/default_image.png" class="mr-3 align-center img-fluid">
                ...
                <!--------------------- end of select for comment section  ------------------------------->
                <?php

                            #--------------------- comment section area  -------------------------------
                            echo '
                        <div class="comment-section">
                        <form action="database/comment.inc.php?id=' . $blog_id . '" method="POST">
                        <div class="card card-container comment-container">
                            <div class="card-header comment-head">
                                <h3>Comment Section</h3>
                            </div>
                            <div class="card-body text-white comment-input">
                                <input type="text" name="commentname" placeholder="Name" class="mb-2">
                                <input type="text" name="commentemail" placeholder="E-Mail" class="mb-2">
                                <textarea name="commenttext" cols="30" rows="3" placeholder="Write here comment" class="mb-2"></textarea>
